...
export function GroupsPage() {
  ...
  const totalGroupSavings = groups.reduce((sum, group) => sum + group.totalSavings, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl mb-1">My Groups</h1>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm">Group Savings</CardTitle>
            <TrendingUp className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl">GHS {totalGroupSavings.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              Combined total
            </p>
          </CardContent>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold">GHS {group.totalSavings.toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Contribution</span>
                  <span>GHS {group.contributionAmount} / {group.contributionFrequency}</span>
                </div>
                <div className="flex justify-between text-sm">
      ...
      {filteredGroups.length === 0 && (
        <div className="text-center py-12">
          ...
          {!searchQuery && (
            <Button onClick={() => setIsCreateModalOpen(true)} className="bg-cyan-500 hover:bg-cyan-600">
