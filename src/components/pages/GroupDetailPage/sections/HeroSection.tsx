...
/**
 * Returns true when the invite button should be shown.
 *
 * Conditions that suppress the invite button:
 *   1. Group is full — no seats available, inviting would be misleading.
 *   2. Group is suspended — the backend will reject any invite link
 *      request for suspended groups, so we skip the button entirely
 *      rather than showing a button that leads to an error.
 *   3. Group is completed or rejected — these are terminal states where
 *      inviting new members makes no sense.
 *
 * Invite IS shown for active (not-full) and pending groups because an
 * admin may want to invite members before the group goes fully active.
 */
function shouldShowInviteButton(group: GroupDetail): boolean {
  const isFull = group.current_members >= group.expected_members;
  if (isFull) return false;

  const nonInvitableStatuses: GroupStatus[] = [
    'suspended',
    'completed',
    'rejected',
  ];
  if (nonInvitableStatuses.includes(group.status)) return false;
...
export function HeroSection({ group, onInviteClick }: HeroSectionProps) {
  ...
  const showInvite = shouldShowInviteButton(group);
      ...
      <div className="relative px-5 py-5 md:px-8 md:py-7 space-y-5">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/*
             * The invite button is conditionally rendered rather than
             * disabled-and-shown so that suspended/completed/rejected groups
             * give no affordance toward inviting.  A disabled button still
             * communicates "this exists but isn't available right now", which
             * would be confusing for terminal states like suspended.
             */}
            {showInvite && (
              <Button
                variant="outline"
        ...
        <div className="space-y-2">
          ...
          {group.status === 'suspended' && (
            <p className="text-xs text-red-600 dark:text-red-400 flex items-center gap-1.5">
              ...
              This group has been suspended. Member invites are disabled.
            </p>
          )}
        </div>
      </div>
...
// // src/components/pages/GroupDetailPage/sections/HeroSection.tsx
