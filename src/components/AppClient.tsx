...
export default function AppClient() {
  // Lazy init: Compute from localStorage on first render
  ...
  const getInitialView = () => {
    const session = localStorage.getItem('snappx_session');
    const userData = localStorage.getItem('snappx_user');
    return session && userData ? 'dashboard' : 'landing';
  };
    ...
    return (
      <AnimatePresence mode="wait">
        <motion.div
          ...
          exit="exit"
          transition={pageTransition}
          className="min-h-screen bg-linear-to-br from-cyan-50 to-teal-50 flex items-center justify-center p-4"
        >
